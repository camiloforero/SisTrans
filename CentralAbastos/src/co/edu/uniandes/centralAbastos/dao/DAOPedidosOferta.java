...
public class DAOPedidosOferta extends ConsultaDAO
{
	...
	private static final String ALL_OFFERS = 
			"SELECT ID_PEDIDO , PO.NOMBRE_PRODUCTO , PO.PESO_CAJA , PO.CANTIDAD , O.PRECIO_X_KG , O.PRECIO_TOTAL , PO.FECHA_ENTREGA , O.FECHA_VENCIMIENTO , O.CORREO_PROVEEDOR , P.CALIFICACION,((prod.PRECIO_KILOGRAMO-O.PRECIO_X_KG)/PROD.PRECIO_KILOGRAMO) AS COEFICIENTE_PRECIOS  FROM PEDIDO_OFERTA PO JOIN OFERTA O ON PO.ID=O.ID_PEDIDO JOIN PROVEEDORES P ON O.CORREO_PROVEEDOR=P.CORREO_USUARIO JOIN PRODUCTOS prod on PO.NOMBRE_PRODUCTO=prod.NOMBRE";	
